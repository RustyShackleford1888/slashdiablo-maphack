...
BreakpointsDisplay* BreakpointsDisplay::display = nullptr; // Corrected initialization
...
BreakpointsDisplay::~BreakpointsDisplay() {
	...
	while (!Hooks.empty()) { // Changed condition
		delete Hooks.front(); // Corrected iterator usage
		Hooks.pop_front(); // Added to remove the element from the list
	}
	Unlock();
...
void BreakpointsDisplay::SetX(unsigned int newX) {
	if (newX <= Hook::GetScreenWidth()) { // Unsigned comparison correction
		Lock();
		x = newX;
		Unlock();
	}
...
void BreakpointsDisplay::SetY(unsigned int newY) {
	if (newY <= Hook::GetScreenHeight()) { // Unsigned comparison correction
		Lock();
		y = newY + 51;
		Unlock();
	}
...
void BreakpointsDisplay::SetXSize(unsigned int newXSize) {
	if (newXSize <= (Hook::GetScreenWidth() - GetX())) { // Unsigned comparison correction
		Lock();
		xSize = newXSize - 38;
		Unlock();
	}
...
void BreakpointsDisplay::SetYSize(unsigned int newYSize) {
	if (newYSize <= (Hook::GetScreenHeight() - GetY())) { // Unsigned comparison correction
		Lock();
		ySize = newYSize;
		Unlock();
	}
...
void BreakpointsDisplay::OnDraw() {
	...
	// Check if the character screen is open
	if (!D2CLIENT_GetUIState(0x02)) {
		return;  // Exit if the character screen is not open
	}
	...
	int currentFCR = static_cast<int>(D2COMMON_GetUnitStat(unit, STAT_FASTERCAST, 0)); // Casting to int
	int currentFCRFrames = 0;
	std::string nextBreakpointFCR;
	int currentFHR = static_cast<int>(D2COMMON_GetUnitStat(unit, STAT_FASTERHITRECOVERY, 0)); // Casting to int
   ...
   // ... existing code ...
   bool BreakpointsDisplay::KeyClick(bool up, unsigned char key, long param) {
